  ...
  return (
    <section id="projects" className="relative py-24 bg-mw-darker">
      ...
      <div className="mw-container relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {projects.map((project, index) => (
            <div 
              key={index} 
              className={cn(
                "mw-card group transition-all duration-700 ease-out transform",
                isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-16",
                isVisible && { 'delay-150': index === 0 || index === 1, 'delay-300': index === 2 || index === 3 }
              )}
            >
              ...
              <div className="p-4 border-t border-mw-green border-opacity-20">
                <div className="flex flex-wrap gap-2 mb-4">
                  {project.tags.map((tag, tagIndex) => (
                    <span key={tagIndex} className="px-2 py-1 bg-mw-green bg-opacity-10 text-mw-green text-xs rounded-sm">
                      {tag}
                    </span>
                  ))}
                </div>
                ...
                <div className="flex justify-end space-x-3">
                  <a 
                    ...
                    className="p-2 text-mw-light hover:text-mw-green transition-colors"
        ...
        >
          <a href="#" className="mw-button inline-flex items-center">
            VIEW ALL PROJECTS
