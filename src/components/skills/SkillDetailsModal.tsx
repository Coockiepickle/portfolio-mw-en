  ...
  return <Dialog open={isOpen} onOpenChange={open => !open && onClose()}>
      <DialogContent className="sm:max-w-[800px] bg-mw-darker border border-mw-green/30 text-white rounded-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
          {skills.map((skill, index) => <div key={index} className="border border-mw-green/20 rounded-md p-0 transition-all hover:border-mw-green/40 bg-black relative overflow-hidden">
              {/* Terminal header */}
              ...
              <div className="p-3 font-mono text-sm">
                ...
                <div className="mb-3 h-1 w-full bg-mw-dark overflow-hidden rounded-full">
                  <div className="h-full bg-gradient-to-r from-mw-green/80 to-mw-green rounded-full" style={{
                width: `${skill.level}%`
              }}></div>
                </div>
                ...
                <div className="mt-3">
                  ...
                  <div className="text-mw-light opacity-90 overflow-y-auto max-h-24 terminal-scrollbar">
                    <CodeCracker text={getSkillDescription(skill.name)} isDecoding={isDecoding} />
                  </div>
                </div>
              </div>
              ...
              <div className="h-px w-full bg-gradient-to-r from-transparent via-mw-green/30 to-transparent mt-3 relative overflow-hidden">
                <div className={cn("absolute top-0 left-0 h-full w-20 bg-gradient-to-r from-transparent via-mw-green/60 to-transparent", "animate-[scan_2s_ease-in-out_infinite_alternate]")}></div>
              </div>
            </div>)}
        </div>
